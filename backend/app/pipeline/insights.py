...
def _format_recs(recs: dict) -> str:
    """Pretty-print segment recommendations as readable multi-line text."""
    if not isinstance(recs, dict) or not recs:
        return ""
    lines = []
    for seg, info in recs.items():
        ...
        lines.append(f"• {seg} [{priority}] - {desc}")
...
def _format_alerts(alerts: list) -> str:
    """Pretty-print priority alerts as readable multi-line text."""
    if not alerts:
        return "No active alerts. Customer base looks healthy."
    lines = []
    for a in alerts:
...
def generate_all_insights(df: pd.DataFrame) -> dict:
    ...
    Cards backed by the LLM (executive_summary, anomaly_insights) are skipped
    when GROQ_API_KEY isn't configured, so the UI doesn't show empty
    "unavailable" placeholders. Rule-based cards (alerts, segment recs) always
    fire and use a fallback when the LLM is unavailable.
    """
    summary  = _build_summary(df)
    insights = {}

    # Executive Summary - LLM only. Skip if unavailable.
    ...
    # Segment recommendations - tries LLM, falls back to rule-based dict.
    recs = _segment_recommendations(summary)
    insights["segment_recommendations"] = {
        "title":    "Segment Recommendations",
        "body":     _format_recs(recs),
        "priority": 2,
    }

    # Anomaly report - LLM only. Skip if unavailable.
    anomaly_text = _anomaly_insights(summary, df)
    if anomaly_text and anomaly_text != "Anomaly report unavailable.":
    ...
    # Priority alerts - rule-based, always fires.
